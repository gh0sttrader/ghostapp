...
const POS: Record<RatingLabel, number> = {
  "Strong sell": 0.02,
  "Sell":        0.28,
  "Neutral":     0.50,
  "Buy":         0.72,
...
export default function AnalystRating({
  label,            // pass this for dummy mode
  score,            // optional normalized 0..1 when real API is wired
}: { label?: RatingLabel; score?: number }) {
  const clamp = (n:number)=>Math.max(0, Math.min(1, n));
  // ✅ label wins; only use score if no label provided
  const pct = label ? POS[label] : clamp(score ?? 0.5);
  const display =
    label ??
    (pct <= .1 ? "Strong sell" : pct < .4 ? "Sell" : pct <= .6 ? "Neutral" : pct < .9 ? "Buy" : "Strong buy");
      ...
      <div className="mt-2 flex flex-col items-center">
        ...
        <div
          ...
          style={{ backgroundImage:
            "linear-gradient(90deg,#4F0F99,#3E179F,#2C1FA7,#2632AF,#2B52BA,#2E74C6,#3199D2,#39BED9,#40E2E0,#45ECCB)" }}
        >
          {display}
        </div>
      </div>
